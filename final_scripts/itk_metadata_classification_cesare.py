...
#modified to give multple labels one hot encoding (in the __getitem__() function)
...
class ITKMetaDataClassification(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #MODIFIED FOR MULTIPLE LABELS######################################
        str = extractlabelsfromfile(self.filenames[index])
        lab = extractlabelfromfile(self.filenames[index])
        ...
        if str and lab is not None:
            #single label
            label = [0] * len(self.classes)
            label[self.class_to_idx[lab]] = 1
            label = np.array(label, dtype=np.float32)
            #multi-labels
            labels = [0] * len(self.classes)
            for i in range(l):
                labels[self.class_to_idx[str.split(',')[i]]] = 1
