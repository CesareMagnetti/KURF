"""
train fetalnav model on ITK images WITH DATA AUGMENTATION
...
tonumpy  = itktransforms.ToNumpy(outputtype='float32')
...
toPIL = torchtransforms.ToPILImage()
randomaffine = torchtransforms.RandomAffine(degrees=10, translate=(0.1,0.1), scale=(0.9, 1.1), shear=5)

#define train and validate transforms
train_transform = torchtransforms.Compose([resample, tonumpy, toPIL, randomaffine, totensor, crop, resize, rescale, flip])
...
modelpath = "/home/cm19/Code/models/vgg13_aug/"
modelname = "vgg13_checkpoint_15.pth.tar"
...
def save_model(model, epoch, optimizer, loss, name = None, path = None):
    ...
    if name == None:
        name = "vgg13_checkpoint_" + str(epoch+1) + ".pth.tar"
    if path == None:
        path = "/home/cm19/Code/models/resnet18"
